    ...
    return (
        {
            ...
            "profile": {"bio": profile.bio, "is_public": profile.is_public},
...
def update_me_public_profile(user: AbstractBaseUser, data: dict) -> dict:
    profile, _ = public_profile_for_user(user.id)
    if "bio" in data:
        profile.bio = str(data.get("bio") or "")[:500]
    if "is_public" in data:
        profile.is_public = bool(data.get("is_public"))
    profile.save()
    return {"bio": profile.bio, "is_public": profile.is_public}
