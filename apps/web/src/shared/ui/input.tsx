...
const Input = React.forwardRef<HTMLInputElement, InputProps>(({ className, type, valid = false, "aria-invalid": ariaInvalid, ...props }, ref) => {
  return (
    <input
      type={type}
      className={cn(
        "flex h-10 w-full rounded-lg border border-border/90 bg-card/80 px-3 py-2 text-start text-sm text-foreground shadow-inner transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground",
        ...
        !ariaInvalid && valid && "border-emerald-500/70 focus-visible:ring-emerald-500/40",
        className,
      )}
      ref={ref}
