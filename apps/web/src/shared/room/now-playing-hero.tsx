...
const ACCENT_RING: Record<string, string> = {
  emerald: "from-brand-muted/95 via-teal-500 to-cyan-500 shadow-[0_20px_50px_-25px_rgba(16,185,129,0.55)]",
  ...
  return (
    ...
    >
      <div className={cn("shrink-0 overflow-hidden bg-gradient-to-br p-[2px]", outer, ring)}>
        <div className={cn("flex h-full w-full items-center justify-center bg-[var(--overlay)] font-bold text-brand", inner)}>
          {letter}
        </div>
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-brand/90">Now playing</p>
        <p className="mt-0.5 line-clamp-2 text-lg font-semibold text-foreground sm:text-xl">{title}</p>
